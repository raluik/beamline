...
const APOLLO_IO_API_KEY = process.env.APOLLO_IO_API_KEY;
...
export async function getCompanyData(options: Options): Promise<EnrichedOrganizationDto[]> {
  ...
  //console.log(JSON.stringify(enrichedOrganizations, null, 2));
  return enrichedOrganizations;
}

async function getOrganizations(options: Options): Promise<OrganizationDto[]> {
  const organizations: OrganizationDto[] = [];

  let response: ListOrganizationResponseDto | null = null;
  do {
    response = await listOrganizations({
      organization_locations: options.locations,
      q_organization_keyword_tags: options.keywords,
...
interface ListOrganizationQueryDto {
...
interface ListOrganizationResponseDto {
...
interface GetEnrichedOrganizationResponseDto {
  readonly organization: EnrichedOrganizationDto;
}

interface EnrichedOrganizationDto extends OrganizationDto {
