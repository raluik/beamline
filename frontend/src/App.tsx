...
const TEST_DATA: CompanyRelevancyResult[] = [
  {
    ...
    "description": "Let's Do It World is the world's biggest civic movement, including over 190 countries and millions of volunteers, whom all stand for a cleaner world. \n\nWorld Cleanup Day is an annual event organized by Let's Do It World. The next World Cleanup Day will take place on the 16th of September 2023, when once again millions of people will unite together to take another step towards a cleaner world.",
    "score": 6,
    "explanation": "The company focuses on civic and social organization and aims to create a cleaner world through cleanup events and promoting environmental change."
  },
  {
    "name": "The Oceancy",
  ...
  },
  {
    ...
    "description": "We are a sustainability reporting and ESG compliance agency providing end-to-end solutions for sustainability reporting, ESG due diligence, drafting of corporate policies, whistleblowing channel services, and related trainings.",
    "score": 5,
    "explanation": "The company provides sustainability reporting and ESG compliance solutions, aligning with the energy efficiency theme."
  },
  {
  ...
  },
  {
    ...
    "description": "Fab City started as a challenge for a city to produce (almost) everything it consumes. The provocation has grown into a global movement, which is facilitated by the Fab City Foundation, a Network of 41 cities and a Collective of experts.\n\nThe Fab City Foundation supports the Fab City global initiative through the development of projects and educational programs that are focused on building the capacity of cities and their communities. Based in e-Estonia, the Foundation is location independent and supports distributed programs and projects the world over.",
    "score": 8.5,
    "explanation": "The company promotes sustainable cities and supports the development of projects and educational programs focused on urban innovation."
  },
  {
  ...
  },
  {
    ...
    "score": 7,
    "explanation": "The company specializes in providing digital construction design solutions for sustainable and energy-efficient buildings in the Nordic and Baltic region."
  }
]
...
function App() {
  ...
  const [relevanceKeywords, setRelevanceKeywords] = useState<string>('sustainability');
  const [minEmployeeCount, setMinEmployeeCount] = useState<string | undefined>(undefined);
  ...
  async function handleSearchPressed(event: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
    ...
    try {
      const res = await axios<{ totalCount: number, organizations: CompanyRelevancyResult[] }>("http://localhost:3000/app/companies", { params: query });
    ...
    <div className='d-flex flex-column gap-3'>
      <Form>
        <Form.Group as={Row} className="mb-3" controlId="formLocations">
          ...
          <Col sm="10">
            <Form.Control disabled={busy} value={locations} />
          </Col>
        </Form.Group>
        ...
        <Form.Group as={Row} className="mb-3" controlId="formKeywords">
          ...
          <Col sm="10">
            <Form.Control disabled={busy} placeholder='preliminary search keywords (for Apollo)' value={keywords} />
          </Col>
        </Form.Group>
        <Form.Group as={Row} className="mb-3" controlId="formRelevanceKeywords">
          ...
          <Col sm="10">
            <Form.Control disabled={busy} placeholder='relevance keywords' value={relevanceKeywords} />
          </Col>
        </Form.Group>
        <Form.Group as={Row} className="mb-3" controlId="formMinEmployeeCount">
          ...
          <Col sm="10">
            <Form.Control disabled={busy} type="number" placeholder='minimum employee count' value={minEmployeeCount} />
          </Col>
        </Form.Group>
        <Form.Group as={Row} className="mb-3" controlId="formMaxEmployeeCount">
          ...
          <Col sm="10">
            <Form.Control disabled={busy} type="number" placeholder='maximum employee count' value={maxEmployeeCount} />
          </Col>
        </Form.Group>
        <Button disabled={busy || keywords.length === 0 || relevanceKeywords.length === 0 || locations.length === 0} variant="primary" type="submit" onClick={handleSearchPressed}>
      ...
      }
      {
        ...
        <span>
          Found {totalCount} companies
        </span>
        : null
      }
      {
        companyResults.length > 0 ?
        <CompanyResultsList companyResults={companyResults}></CompanyResultsList>
        : null
      }
    </div>
  )
